...
OCaml and Java are _statically typed_ languages, meaning every binding has a
type that is determined at _compile time_&mdash;that is, before any part of the
...
Unlike Java, OCaml is _implicitly typed_, meaning programmers rarely need to
...
sophisticated because it must infer what the _type annotations_ "would have
...
in practice they are often merged into a single procedure called _type
reconstruction_.
...
- For each `let` definition, analyze the definition to determine _constraints_
...
least helpful (see _Real World OCaml_ chapter 22, "Type inference", for
...
application. Let's introduce a new syntactic class `n` for _names_, which
...
How would _you_ mentally infer the type of `fun x -> 1 + x`, or rather,
...
we say that the substitutions `{1 / x}` and `{2 / y}` together _unify_ that set
...
evaluation, we'll write `{t / 'x}` for the _type substitution_ that maps type
...
substitution that is their _sequential composition_, which is defined as
follows:
...
A substitution _unifies_ a constraint `t_1 = t_2` if `t_1 S` results in the same
...
- If `C` contains at least one constraint `t1 = t2` and possibly some other
  ...
  - If `t1` and `t2` are both the same simple type&mdash;i.e., both the same
    ...
    `unify(C')`. _In this case, the constraint contained no useful
    information, so we're tossing it out and continuing._

  - If `t1` is a type variable `'x` and `'x` does not occur in `t2`, then let
    `S = {t2 / 'x}`, and return `S; unify(C' S)`. _In this case, we are
    eliminating the variable `'x` from the system of equations, much like
    Gaussian elimination in solving algebraic equations._

  - If `t2` is a type variable `'x` and `'x` does not occur in `t1`, then let
    `S = {t1 / 'x}`, and return `S; unify(C' S)`. _This is an elimination
    like the previous case._

  - If `t1 = i1 -> o1` and `t2 = i2 -> o2`, where `i1`, `i2`, `o1`, and `o2`
    are types, then `unify(i1 = i2, o1 = o2, C')`. _In this case, we break one
    constraint down into two smaller constraints and add those constraints
    back in to be further unified._
...
algorithm produces is the _most general unifier_, in the sense that if
...
_principal_ type for the expression, meaning that if `e` also has type `t` for
...
It requires us to introduce a new kind of type: a _type scheme_. Type schemes
resemble _universal quantification_ from mathematical logic. For example, in
...
The solution adopted by OCaml and related languages is called the _value
restriction:_ the type system is designed to prevent a polymorphic mutable value
...
Here we are using _subtype polymorphism_ to assign an array of `Rabbit` objects
to an `Animal[]` reference. That's not the same as _parametric polymorphism_ as
